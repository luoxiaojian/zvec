 ...
 public:
  ...
  static Result<core_interface::QuantizerType> convert_to_engine_quantize_type(
    ...
    switch (quantize_type) {
      ...
      case QuantizeType::UNIFORM_INT8:
        return core_interface::QuantizerType::kUniformInt8;
  ...
  convert_to_engine_index_param(const FieldSchema &field_schema) {
    ...
    switch (field_schema.index_params()->type()) {
      ...
      case IndexType::VAMANA: {
        ...
        index_param_builder->WithReversePruneBatchSize(
            db_index_params->reverse_prune_batch_size());
        index_param_builder->WithBuildPrefetchOffset(
            db_index_params->build_prefetch_offset());
        index_param_builder->WithBuildPrefetchLines(
            db_index_params->build_prefetch_lines());
        index_param_builder->WithUseOptimizedBuild(
            db_index_params->use_optimized_build());
