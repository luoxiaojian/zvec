...
// ==========================================================================
// Split-layout variant: the 20-byte metadata tail lives in a separate flat
// array, so `vectors[i]` points to the pure int8 body of length
// (dim - 20), and `side_data[i]` points to the metadata block of vector i.
// The query still has its own 20-byte tail appended (same as the non-split
// path).  Internally identical to `squared_euclidean_int8_batch_distance`,
// except the per-vector `m_tail` pointer is taken from `side_data[i]`
// rather than `vectors[i] + original_dim`.
// ==========================================================================
void squared_euclidean_int8_batch_distance_split(const void *const *vectors,
  ...
  for (; i + batch_size <= n; i += batch_size) {
    ...
    for (size_t j = 0; j < batch_size; ++j) {
      if (i + j + batch_size * prefetch_step < n) {
        size_t prefetch_idx = i + j + batch_size * prefetch_step;
        prefetch_ptrs[j] = vectors[prefetch_idx];
        // Pull side_data into L1 alongside the vector body.  Unlike the
        // non-split layout where the 20B tail is adjacent to the body and
        // naturally rides on the body prefetch, split-layout side_data
        // lives in a separate flat array and would otherwise be a cold
        // miss on every neighbor access (especially painful on large
        // dims like GIST-960 where the body prefetch cannot mask it).
        _mm_prefetch(reinterpret_cast<const char *>(side_data[prefetch_idx]),
                     _MM_HINT_T0);
      } else {
        prefetch_ptrs[j] = nullptr;
      }
    ...
    internal::ip_int8_batch_avx512_vnni_impl<batch_size>(
        query, &vectors[i], prefetch_ptrs, original_dim, ip_dists.data());
    for (size_t j = 0; j < batch_size; ++j) {
      const float *m_tail = reinterpret_cast<const float *>(side_data[i + j]);
      float mA = m_tail[0];
      float mB = m_tail[1];
      float mS = m_tail[2];
      float mS2 = m_tail[3];
  ...
  for (; i < n; ++i) {
    ...
    internal::ip_int8_batch_avx512_vnni_impl<1>(
        query, &vectors[i], prefetch_ptrs, original_dim, &ip_dist);
    const float *m_tail = reinterpret_cast<const float *>(side_data[i]);
    float mA = m_tail[0];
    float mB = m_tail[1];
    float mS = m_tail[2];
    float mS2 = m_tail[3];
