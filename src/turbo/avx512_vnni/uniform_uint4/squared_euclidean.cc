...
inline __m512i Accumulate(__m512i sum, __m512i packed,
                          __m512i query_low, __m512i query_high,
                          __m512i nibble_mask) {
  const __m512i low = _mm512_and_si512(packed, nibble_mask);
  const __m512i high =
      _mm512_and_si512(_mm512_srli_epi16(packed, 4), nibble_mask);
  const __m512i low_delta =
      _mm512_abs_epi8(_mm512_sub_epi8(low, query_low));
  const __m512i high_delta =
      _mm512_abs_epi8(_mm512_sub_epi8(high, query_high));
  sum = _mm512_dpbusd_epi32(sum, low_delta, low_delta);
  return _mm512_dpbusd_epi32(sum, high_delta, high_delta);
}

static ailego_force_inline void Distance(
    const uint8_t *lhs, const uint8_t *rhs, size_t encoded_dimension,
    float *distance) {
...
static ailego_force_inline void DistanceFour(
    const void **vectors, const uint8_t *query, size_t encoded_dimension,
    float *distances) {
  const __m512i mask = _mm512_set1_epi8(0x0f);
  ...
  for (; offset + 64 <= encoded_dimension; offset += 64) {
    ...
    for (size_t lane = 0; lane < 4; ++lane) {
      ...
      sums[lane] =
          Accumulate(sums[lane], _mm512_loadu_si512(row + offset), query_low,
                     query_high, mask);
    }
  }
  for (size_t lane = 0; lane < 4; ++lane) {
