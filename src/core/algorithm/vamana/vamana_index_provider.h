...
class VamanaIndexProvider : public IndexProvider {
 ...
 public:
  ...
  size_t count(void) const override { return entity_->doc_cnt(); }

  size_t dimension(void) const override { return meta_.dimension(); }

  IndexMeta::DataType data_type(void) const override {
    return meta_.data_type();
  }

  size_t element_size(void) const override { return meta_.element_size(); }
 ...
 private:
  class Iterator : public IndexProvider::Iterator {
   public:
    ...
    uint64_t key(void) const override { return entity_->get_key(cur_id_); }

    void next(void) override {
      cur_id_ = get_next_valid_id(cur_id_ + 1);
    }

    void reset(void) { cur_id_ = get_next_valid_id(0); }
