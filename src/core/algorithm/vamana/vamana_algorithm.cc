...
int VamanaAlgorithm<EntityType>::search(VamanaContext *ctx) const {
  ...
  greedy_search(entry_point, ctx, GreedySearchMode::kFastQuery);
...
// mmap/contiguous variant: resolve vector and optional extra-values pointers
...
void fast_greedy_search(const EntityType &entity, HeapType &pool,
    ...
    if constexpr (std::is_same_v<HeapType, LinearPool<float>>) {
      for (uint32_t i = 0; i < count; ++i) {
        if constexpr (FullGraphPrefetch) {
          bool rewound;
          if (pool.insert(static_cast<int>(neighbor_ids[i]), dists[i],
                          &rewound) &&
              rewound) {
  ...
  if constexpr (CachedGreedyEntry) {
    ...
    const auto neighbors = entity.get_neighbors_typed(start);
    if (cached_row != start) {
      compute_neighbor_row(neighbors);
      if (ailego_unlikely(dc.error())) {
        return;
  ...
  if (!VisitImpl::visited(visit_ctx, start)) {
    VisitImpl::set_visited(visit_ctx, start);
...
template <bool HasExtraValues, bool UseLinearPool, bool CachedGreedyEntry,
          bool FullGraphPrefetch, typename EntityType>
 ...
 private:
  ...
  void run_with_heap(typename VisitImpl::Context *visit_ctx) const {
    if constexpr (std::is_same_v<HeapType, BlockHeap>) {
      ...
      fast_greedy_search<HasExtraValues, CachedGreedyEntry, FullGraphPrefetch,
                         EntityType, BlockHeap, VisitImpl>(
      ...
      auto &pool = ctx->pool();
      fast_greedy_search<HasExtraValues, CachedGreedyEntry, FullGraphPrefetch,
                         EntityType, LinearPool<float>, VisitImpl>(
          entity, pool, dc, pool_capacity, entry_point, prefetch_lines,
          ctx->po(), enforce_scan_limit, ctx, visit_ctx);
      if (!ctx->direct_pool_result()) {
        copy_pool_to_topk(pool, topk_heap);
...
// batch_dist.
// ============================================================================
template <bool HasExtraValues, typename EntityType, typename MemBlockType,
          typename FilterFn>
void dual_heap_greedy_search(const EntityType &entity, VamanaContext *ctx,
  ...
  const uint32_t prefetch_lines =
      ctx->pl() > 0 ? std::min(ctx->pl(), body_lines) : body_lines;
...
void VamanaAlgorithm<EntityType>::greedy_search(node_id_t entry_point,
  ...
  if (mode == GreedySearchMode::kDualHeap || index_filter.is_valid()) {
    auto run_with_filter = [&](auto &&filter) {
      if (has_extra_values) {
        dual_heap_greedy_search<true, EntityType, MemBlockType>(
            entity, ctx, dc, entry_point,
            std::forward<decltype(filter)>(filter));
      } else {
        dual_heap_greedy_search<false, EntityType, MemBlockType>(
            entity, ctx, dc, entry_point,
            std::forward<decltype(filter)>(filter));
      }
    };
    ...
    const bool build_search = mode == GreedySearchMode::kFastBuild;
    const uint32_t pool_capacity =
        build_search ? static_cast<uint32_t>(ctx->topk_heap().limit())
                     : std::max(static_cast<uint32_t>(ctx->topk()), ctx->ef());
    const bool avx2_ok =
    ...
    if (has_extra_values) {
      dual_heap_greedy_search<true, EntityType, MemBlockType>(
          entity, ctx, dc, entry_point, filter);
    } else {
      dual_heap_greedy_search<false, EntityType, MemBlockType>(
          entity, ctx, dc, entry_point, filter);
    }
