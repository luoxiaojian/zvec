...
struct VamanaHeader {
  ...
  size_t max_degree() const { return graph.max_degree; }
  size_t vector_size() const { return graph.vector_size; }
  size_t search_list_size() const { return graph.search_list_size; }
  size_t max_occlusion_size() const { return graph.max_occlusion_size; }
  float alpha() const { return graph.alpha; }
  node_id_t entry_point() const { return graph.entry_point; }
  node_id_t doc_cnt() const { return graph.doc_count; }
...
class VamanaEntity {
 public:
  VamanaEntity() {}
  VamanaEntity(const VamanaHeader &hd) { header_ = hd; }
  ...
  inline size_t max_degree() const { return header_.graph.max_degree; }
  inline node_id_t *mutable_doc_cnt() { return &header_.graph.doc_count; }
  inline node_id_t doc_cnt() const { return header_.graph.doc_count; }
  inline float alpha() const { return header_.graph.alpha; }
  ...
  inline node_id_t entry_point() const { return header_.graph.entry_point; }
  inline size_t vector_size() const { return header_.graph.vector_size; }
  inline size_t node_size() const { return header_.graph.node_size; }

  void set_vector_size(size_t size) { header_.graph.vector_size = size; }
  void set_max_degree(uint32_t val) { header_.graph.max_degree = val; }
 ...
 protected:
  inline const VamanaHeader &header() const { return header_; }
  inline VamanaHeader *mutable_header() { return &header_; }
  inline size_t header_size() const { return sizeof(header_); }

  void set_node_size(size_t size) { header_.graph.node_size = size; }

  int64_t dump_segments(const IndexDumper::Pointer &dumper,
                        key_t *keys) const;
