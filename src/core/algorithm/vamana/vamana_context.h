...
class VamanaContext : public IndexContext {
 public:
  ...
  // graph schema.  Returned values are concrete and can be consumed by the hot
  // search loop without auto/manual branches.
  ...
  inline bool direct_pool_result() const {
    return direct_pool_result_;
  }
  ...
  // buffers in the context avoids allocating and freeing three (or four for
  // metrics with extra values) max-degree arrays for every query.
  ...
  inline void set_build_fast_search(bool enabled) {
    build_fast_search_ = enabled;
  }
 ...
 private:
  ...
  bool direct_pool_result_{false};
