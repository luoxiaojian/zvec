...
std::pair<uint32_t, uint32_t> VamanaContext::resolve_query_prefetch(
  ...
  const uint32_t body_lines = static_cast<uint32_t>(
      (vector_data_size + kPrefetchCacheLineBytes - 1) /
  ...
  if (requested_lines == kDefaultPrefetchLines) {
    ...
    resolved_lines =
        std::min({requested_lines, body_lines, kPrefetchMaxValue});
  }
  ...
  if (requested_offset == kDefaultPrefetchOffset) {
    ...
    const uint32_t budget_offset = static_cast<uint32_t>(std::max<uint64_t>(
        1, kPrefetchBudgetBytes / bytes_per_neighbor));
    resolved_offset =
        std::min({budget_offset, max_degree, kPrefetchMaxValue});
  } else {
    resolved_offset =
