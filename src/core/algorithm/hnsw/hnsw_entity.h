...
struct MmapMemoryBlock {
  ...
  void reset(void *data) { data_ = data; }
...
struct BufferPoolMemoryBlock {
  ...
  ~BufferPoolMemoryBlock() { release(); }

  const void *data() const { return data_; }

  void reset(ailego::VecBufferPoolHandle *handle, size_t block_id,
             void *data) {
    release();
...
struct NeighborsT {
  ...
  const node_id_t &operator[](size_t idx) const { return data[idx]; }
