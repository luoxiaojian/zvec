...
int HnswContiguousStreamerEntity::build_contiguous_memory() {
  ...
  for (size_t chunk_idx = 0; chunk_idx < chunks.size(); ++chunk_idx) {
    ...
    for (uint32_t i = 0; i < count_in_chunk; ++i) {
      ...
      std::memcpy(graph_base_ + global_id * graph_stride_,
                  node_src + vec_size, graph_stride_);
    }
  }
  ...
  LOG_INFO(
      ...
      vector_memory_size, graph_memory_size, upper_memory_size,
      total_docs, chunks.size(), upper_chunks.size());
