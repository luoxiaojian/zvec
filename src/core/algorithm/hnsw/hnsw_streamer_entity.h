...
class HnswStreamerEntity : public HnswEntity {
 ...
 protected:
  ...
  //! Sync chunks and return the chunk base addresses for mmap mode.
  //! After calling this, node_chunks_ and upper_neighbor_chunks_ are
  //! guaranteed to contain the requested chunk.
  inline const std::vector<Chunk::Pointer> &node_chunks() const {
    return node_chunks_;
  }
  inline const std::vector<Chunk::Pointer> &upper_neighbor_chunks() const {
    return upper_neighbor_chunks_;
  }
...
class HnswMmapStreamerEntity : public HnswStreamerEntity {
 public:
  ...
  inline TypedNeighbors get_neighbors_typed(level_t level, node_id_t id) const {
    ...
    uint32_t offset = (((meta->index) & upper_neighbor_mask()) + level - 1) *
                      upper_neighbor_size();
  ...
  inline int get_vector_typed(const node_id_t *ids, uint32_t count,
    ...
    vec_blocks.resize(count);
    for (auto i = 0U; i < count; ++i) {
      uint32_t chunk_idx = ids[i] >> node_index_mask_bits();
      uint32_t offset = (ids[i] & node_index_mask()) * node_size();
      const char *base = get_node_chunk_base(chunk_idx);
 ...
 private:
  ...
  void sync_upper_neighbor_chunk_bases(uint32_t chunk_idx) const {
    ...
    const auto &chunks = upper_neighbor_chunks();
