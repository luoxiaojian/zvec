...
//
// Derived from pyglass' BlockHeap (see
// /root/vector_databases/search/pyglass/include/block_heap.h). The graph
// prefetch is intentionally omitted: the call-site is expected to issue the
// neighbor-array prefetch itself (Vamana's greedy_search already does so).
//
// AVX2 requirement
// ----------------
