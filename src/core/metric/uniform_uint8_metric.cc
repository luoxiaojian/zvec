...
void UniformUint8SquaredEuclideanPreprocessedQuery(const void *a, const void *b,
                                                   size_t dim,
                                                   float *distance) {
  const size_t orig_dim = original_dim(dim);
  const auto *lhs = reinterpret_cast<const uint8_t *>(a);
  ...
  for (size_t i = 0; i < orig_dim; ++i) {
    ip_shifted += static_cast<int>(lhs[i]) * static_cast<int>(rhs[i]);
  }
  const int32_t *lhs_tail = tail(a, orig_dim);
  const int32_t *rhs_tail = tail(b, orig_dim);
...
void UniformUint8SquaredEuclideanPreprocessedQueryBatch(
    ...
    float *distances) {
  for (size_t i = 0; i < n; ++i) {
    UniformUint8SquaredEuclideanPreprocessedQuery(vectors[i], query, dim,
                                                  distances + i);
  }
}
#endif
...
class UniformUint8Metric : public IndexMetric {
 public:
  explicit UniformUint8Metric(bool query_mode = false)
  ...
  MatrixDistance distance_matrix(size_t m, size_t n) const override {
    if (m == 1 && n == 1) {
      ...
      auto turbo_ret = turbo::get_distance_func(
          ...
          query_mode_ ? turbo::QuantizeType::kUniformUint8Query
...
#if ZVEC_UNIFORM_UINT8_QUERY_PREPROCESS
      if (query_mode_) {
        return UniformUint8SquaredEuclideanPreprocessedQuery;
      }
#endif
...
#if ZVEC_UNIFORM_UINT8_QUERY_PREPROCESS
    auto metric = std::make_shared<UniformUint8Metric>(true);
    metric->init(meta_, params_);
    return metric;
#else
    return nullptr;
#endif
  ...
  DistanceBatchQueryPreprocessFunc get_query_preprocess_func() const override {
    ...
    return turbo::get_query_preprocess_func(
        turbo::MetricType::kSquaredEuclidean, turbo::DataType::kInt8,
        turbo::QuantizeType::kUniformUint8Query);
#else
    return nullptr;
#endif
  }
