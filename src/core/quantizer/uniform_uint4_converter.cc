...
class UniformUint4StreamingConverter : public IndexConverter {
 public:
  UniformUint4StreamingConverter(IndexMeta::DataType /*dst_type*/) {}
  ~UniformUint4StreamingConverter() override = default;
  ...
  int init(const IndexMeta &index_meta, const ailego::Params &params) override {
    ...
    *stats_.mutable_transformed_count() = 0;
    ...
    meta_.set_converter("UniformUint4StreamingConverter", 0, params);
    ...
    if (has_minimum && has_range && range_ > 0.0f &&
  ...
  int train(IndexHolder::Pointer holder) override {
    ...
    if (record_count == 0) {
      LOG_ERROR("UniformUint4StreamingConverter: empty training set");
      return IndexError_InvalidArgument;
    }
    if (record_count == std::numeric_limits<size_t>::max() ||
        record_count > std::numeric_limits<size_t>::max() /
                           original_dimension_) {
      LOG_ERROR("UniformUint4StreamingConverter: invalid training count");
      return IndexError_InvalidArgument;
    }
    ...
    if (holder->multipass()) {
      ...
      size_t tail = static_cast<size_t>(static_cast<float>(value_count) *
                                        0.01f) +
                    size_t{1};
      ...
      for (int shift = 24; shift >= 0; shift -= 8) {
        ...
        if (!iter) {
          LOG_ERROR("UniformUint4StreamingConverter: iterator unavailable");
        ...
        if (actual_records != record_count ||
          ...
          LOG_ERROR("UniformUint4StreamingConverter: radix selection failed");
          return IndexError_Runtime;
        }
      }
      ...
      LOG_WARN(
          "UniformUint4StreamingConverter: one-pass holder; using global "
    ...
    *stats_.mutable_trained_count() = record_count;
    ...
    LOG_INFO(
        "UniformUint4StreamingConverter train done: costtime %zums, "
 ...
 private:
  void SetReformerParams() {
    ...
    reformer_params.set(UNIFORM_UINT4_REFORMER_ORIGINAL_DIMENSION,
                        original_dimension_);
    meta_.set_reformer("UniformUint4StreamingReformer", 0, reformer_params);
  }
    ...
    class Iterator : public IndexHolder::Iterator {
     public:
      Iterator(const UniformUint4Holder *owner,
          ...
          : owner_(owner), buffer_(owner->encoded_dimension_, 0),
...
INDEX_FACTORY_REGISTER_CONVERTER_ALIAS(UniformUint4StreamingConverter,
                                       UniformUint4StreamingConverter,
