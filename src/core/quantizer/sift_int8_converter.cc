...
/*! Converter for SIFT Int8 Quantization (scale=1, with sq_sum extra field)
 *
 * Designed for SIFT-like datasets where all values are non-negative integers
 * fitting in [0, 255]. Uses scale=1 for lossless quantization:
 ...
 */
class SiftInt8StreamingConverter : public IndexConverter {
 public:
  SiftInt8StreamingConverter(IndexMeta::DataType /*dst_type*/) {}

  ~SiftInt8StreamingConverter() override {}
  ...
  int init(const IndexMeta &index_meta, const ailego::Params &params) override {
    ...
    *stats_.mutable_transformed_count() = 0;
    ...
    meta_.set_converter("SiftInt8StreamingConverter", 0, params);
    ...
    metric_params.set(SIFT_INT8_METRIC_ORIGIN_METRIC_NAME,
                      index_meta.metric_name());
    meta_.set_metric("SiftInt8", 0, metric_params);

    ailego::Params reformer_params;
    meta_.set_reformer("SiftInt8StreamingReformer", 0, reformer_params);
  ...
  int train(IndexHolder::Pointer holder) override {
    if (!holder) {
      LOG_ERROR("SiftInt8StreamingConverter: null holder in train");
    ...
    if (!iter) {
      LOG_ERROR("SiftInt8StreamingConverter: failed to create iterator");
    ...
    for (; iter->is_valid(); iter->next()) {
      ...
      for (size_t i = 0; i < original_dimension_; ++i) {
        ...
        if (!std::isfinite(v)) {
          LOG_ERROR(
              "SiftInt8StreamingConverter: non-finite value in training "
    ...
    if (*stats_.mutable_trained_count() == 0) {
      LOG_ERROR("SiftInt8StreamingConverter: empty training set");
      return IndexError_InvalidArgument;
    }

    // For SIFT data: scale=1, bias shifts global_min to -128 (or -127).
    // This maps [global_min, global_min+255] -> [-128, 127].
    bias_ = -std::round(global_min) - 128.0f;

    LOG_INFO(
        "SiftInt8StreamingConverter train done: costtime %zums, "
        "global_min=%f, global_max=%f, bias=%f",
        (size_t)timer.milli_seconds(), global_min, global_max, bias_);

    ailego::Params reformer_params;
    reformer_params.set(SIFT_INT8_REFORMER_BIAS, bias_);
    meta_.set_reformer("SiftInt8StreamingReformer", 0, reformer_params);

    ailego::Params conv_params = meta_.converter_params();
    conv_params.set(SIFT_INT8_REFORMER_BIAS, bias_);
  ...
  int transform(IndexHolder::Pointer holder) override {
    ...
    *stats_.mutable_transformed_count() += holder->count();
    holder_ =
        std::make_shared<SiftInt8Holder>(holder, original_dimension_, bias_);
    return 0;
  }

  int dump(const IndexDumper::Pointer & /*dumper*/) override { return 0; }

  const Stats &stats(void) const override { return stats_; }

  IndexHolder::Pointer result(void) const override { return holder_; }

  const IndexMeta &meta(void) const override { return meta_; }

 private:
  //! IndexHolder that applies SIFT int8 quantization on-the-fly
  class SiftInt8Holder : public IndexHolder {
   public:
    class Iterator : public IndexHolder::Iterator {
     public:
      Iterator(const SiftInt8Holder *owner,
      ...
      const void *data(void) const override { return buffer_.data(); }

      bool is_valid(void) const override { return front_iter_->is_valid(); }

      uint64_t key(void) const override { return front_iter_->key(); }
     ...
     private:
      ...
      const SiftInt8Holder *owner_{nullptr};
      std::vector<int8_t> buffer_{};
      IndexHolder::Iterator::Pointer front_iter_{};
    };

    SiftInt8Holder(IndexHolder::Pointer front, size_t original_dim, float bias)
        : front_(std::move(front)),
          original_dim_(original_dim),
          bias_(bias) {}

    size_t count(void) const override { return front_->count(); }

    size_t dimension(void) const override {
      return original_dim_ + sizeof(float);
    }

    size_t output_dimension(void) const { return dimension(); }
    ...
    bool multipass(void) const override { return front_->multipass(); }

    IndexHolder::Iterator::Pointer create_iterator(void) override {
      auto iter = front_->create_iterator();
      return iter ? IndexHolder::Iterator::Pointer(
                        new SiftInt8Holder::Iterator(this, std::move(iter)))
...
INDEX_FACTORY_REGISTER_CONVERTER_ALIAS(SiftInt8StreamingConverter,
                                       SiftInt8StreamingConverter,
