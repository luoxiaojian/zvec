...
ailego::JsonObject VamanaIndexParam::SerializeToJsonObject(
  ...
  if (!omit_empty_value || two_pass_build != kDefaultVamanaTwoPassBuild) {
  ...
  if (!omit_empty_value || use_bulk_build != kDefaultVamanaUseBulkBuild) {
    json_obj.set("use_bulk_build", ailego::JsonValue(use_bulk_build));
  }
  return json_obj;
}
...
bool VamanaIndexParam::DeserializeFromJsonObject(
  ...
  DESERIALIZE_VALUE_FIELD(json_obj, use_bulk_build);
