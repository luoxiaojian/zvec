...
int Index::Open(const std::string &file_path, StorageOptions storage_options) {
  ...
  if (core::IndexHelper::DeserializeFromStorage(storage_.get(), &disk_meta) ==
          0 &&
      !disk_meta.reformer_name().empty()) {
    ...
    if (reformer_mismatch || metric_mismatch || dim_mismatch) {
      ...
      const ailego::Params streamer_params = disk_meta.streamer_params();
